...
#
createCorrelationMatrices = function ()
{
	...
	# it will store the maximum among min(abs(corr_matrix)), computed
	# for every corr_matrix in corr_matrix_list
	maxmin = -1
	...
	for(k in 1:length(names_session))
	{
		...
		maxmin = max(maxmin, min(abs(corr_matrix)))
